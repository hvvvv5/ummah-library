...
export function AyahTafsir({ sura, aya }: { sura: number; aya: number }) {
  ...
  const header = editions.length > 1
    ...
    : t("ayahTafsir.header", { name: locale === "en" ? nameOf(editions[0] ?? "") : t("tafsir.selectedEdition") });
          ...
          {editions.map((id) => {
            ...
            return (
              <View key={id} style={editions.length > 1 ? styles.section : undefined}>
                {editions.length > 1 && <Text style={styles.sectionName}>{locale === "en" ? nameOf(id) : t("tafsir.selectedEdition")}</Text>}
